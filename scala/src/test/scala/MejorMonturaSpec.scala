...
import modelo.{Arma, MejorMontura, Vikingo}
...
class MejorMonturaSpec extends AnyFreeSpec {
  ...
  "MejorMontura" - {
    ...
    "en una carrera" in {
      val resultado = MejorMontura(aumentoDeCarnes, List(dragonDylan, dragonConan), Carrera)
      resultado shouldEqual aumentoDeCarnes.montar(dragonConan)
    }
    ...
    "en una pesca" in {
      ...
      resultado shouldEqual javierDelay
    }
    ...
    "en una carrera decidiendo entre Conan y 10 clones más de Conan" in {
      ...
      val resultado = MejorMontura(javierDelay, clonesConan :+ dragonConan, Carrera)
      resultado shouldEqual javierDelay.montar(dragonConan)
    }
  }
}
